...
const Home = ({ navigation }) => {
  ...
  const [openSelection, setOpenSelection] = useState(false); // State for selection modal visibility
  ...
  const cards = [
    {
      ...
      icon: <ProgressReport width={30} height={30} />,
      screen: 'studentReport',
      path: 'children',
    },
    {
      title: 'View Schedule',
      icon: <Schedule width={30} height={30} />,
      screen: 'studentSchedule',
      path: 'children',
    },
    {
      title: 'View Attendance',
      icon: <ViewAttendance width={30} height={30} />,
      screen: 'studentAttendance',
      path: 'children',
    },
    {
      title: 'Pay Fee',
      icon: <PayFee width={30} height={30} />,
      screen: 'fee',
      path: 'root',
    },
    {
      title: 'Book Compensation',
      icon: <BookCompensation width={30} height={30} />,
      screen: 'compensation',
      path: 'root',
    },
    {
      title: 'Report a Leave',
      icon: <ReportLeave width={30} height={30} />,
      screen: 'leaveApplication',
      path: 'root',
    },
    {
      title: 'Notifications',
      icon: <Notifications width={30} height={30} />,
      screen: 'notifications',
      path: 'root',
    },
    {
      title: 'Testimonials',
      icon: <Testimonials width={30} height={30} />,
      screen: 'testimonials',
      path: 'root',
    },
    {
  ...
  const logOutHandler = async () => {
    ...
    // Filter out the current FCM token from the global data
  ...
  const onRefresh = useCallback(() => {
    ...
    dispatch(globalData(user?.id))
      ...
      .catch(() => {
        setRefreshing(false); // Ensure refreshing state is set to false even if there's an error
      });
  }, []);

  const renderItem = ({ item }) => {
    // const navigation = useNavigation()
    const src = item?.picture
      ? { uri: getImage(item?.picture) }
      : require('../../images/profileAvatar.png');
    ...
    return (
      <TouchableOpacity
        ...
        <View style={styles.item}>
          ...
          <Text
            style={[styles.nameText, { fontFamily: FontFamily.interSemiBold }]}>
            {item.fullName}
          </Text>
          <Text
            style={[
            ...
            {item.StudentYear.name} - {item.feeChargedBy}
          </Text>
        </View>
      </TouchableOpacity>
    );
      ...
      {!!globaldata?.students && !!user?.email ? (
        <ScrollView
            ...
            children={
              <View style={{ width: '100%' }}>
                <DropdownComponent
                  ...
                  disable={false}
                  data={globaldata?.students.map(elem => ({
                    name: elem?.fullName,
                    value: elem?.id,
                  }))}
          ...
          <View style={styles.profileContainer}>
            ...
            <View style={[styles.profileRowContainer, GlobalStyles.p_10]}>
              <View>
                <Text style={[styles.CompText]}>
                  Enrolled Children (
                  {globaldata?.students ? globaldata?.students?.length : 0})
                </Text>
              </View>
              <TouchableOpacity
                activeOpacity={0.7}
                onPress={() =>
                  navigation.navigate('root', { screen: 'children' })
                }>
                <Text style={[styles.CompText, { color: Color.primary }]}>
                  see all
                </Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={[styles.profileRowContainer, GlobalStyles.p_10]}>
              <Text style={[styles.CompText, { marginVertical: 10 }]}>
                More With Prime Tuition
              </Text>
            </View>
            <View
              style={{
